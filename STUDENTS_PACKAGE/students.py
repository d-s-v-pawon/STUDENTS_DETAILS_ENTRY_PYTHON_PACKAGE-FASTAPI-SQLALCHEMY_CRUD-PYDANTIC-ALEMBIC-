...
class StudentEntry(StudentsBase):
    SCHOOL_NAMES : str
    STANDARDS : int
    SECTIONS : str

class StudentDetails(StudentEntry):
    ROLL_NUMBERS : int
    CONTACT_NUMBERS : int
    PERCENTAGES : float
    class config:
...
def create_student(db : Session , STU : StudentDetails):
    db_student = STUDENT(NAMES = STU.NAMES ,
                        GENDERS = STU.GENDERS ,
                        AGES = STU.AGES , 
                        SCHOOL_NAMES = STU.SCHOOL_NAMES , 
                        STANDARDS = STU.STANDARDS , 
                        SECTIONS = STU.SECTIONS ,
                        ROLL_NUMBERS = STU.ROLL_NUMBERS,
                        CONTACT_NUMBERS = STU.CONTACT_NUMBERS,
                        PERCENTAGES = STU.PERCENTAGES)
    db.add(db_student)
    db.commit()
    return db_student

def get_students_crud(db:Session,skip:int=0,limit:int=100):
   return db.query(STUDENT).order_by(STUDENT.ROLL_NUMBERS).offset(skip).limit(limit).all()
...
@app.post("/Students_Details_Enrollment",response_model = StudentDetails)
def enter_student(STU : StudentDetails , session : Session = Depends(get_db)):
    return create_student(db=session,STU=STU)

@app.get("/All_Students",response_model=list[StudentDetails])
def get_students(skip : int = 0 , limit : int = 100 ,session : Session = Depends(get_db)):
    students = get_students_crud(db=session,skip=skip,limit=limit)
    return students
